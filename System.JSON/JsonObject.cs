...
using JsonPairEnumerable = System.Collections.Generic.IEnumerable<System.Collections.Generic.KeyValuePair<string, JWT.JSON.JsonValue>>;
...
namespace JWT.JSON
{
	public class JsonObject : JsonValue, IDictionary<string, JsonValue>, ICollection<JsonPair>
	{
		...
		{
			map = new SortedDictionary<string, JsonValue> (System.StringComparer.Ordinal);
		...
		public override void Save (System.IO.Stream stream)
		{
			...
			foreach (JsonPair pair in map) {
				stream.WriteByte ((byte) '"');
				byte [] bytes = System.Text.Encoding.UTF8.GetBytes (EscapeString (pair.Key));
