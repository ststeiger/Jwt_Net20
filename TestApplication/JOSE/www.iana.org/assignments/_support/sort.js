...
function sortTable()
{
    ...
    function continuation() {
        if ( $(el).hasClass('sorted') ) {
            ...
            el.getElementsByTagName('img')[0].setAttribute( 'src', 'https://www.iana.org/assignments/_support/sort_up.gif' );
            reverseRows( el.sortTable );
        }
        else if ( $(el).hasClass('reversed') ) {
            $(el).removeClass('reversed');
            $(el).addClass('sorted');
            el.getElementsByTagName('img')[0].setAttribute( 'src', 'https://www.iana.org/assignments/_support/sort_down.gif' );
            reverseRows( el.sortTable );
        }
        else {
            var rows = [];
            ...
            for ( var i = 0; i < imgs.length; ++i )
                imgs[i].setAttribute( 'src', 'https://www.iana.org/assignments/_support/sort_none.gif' );
