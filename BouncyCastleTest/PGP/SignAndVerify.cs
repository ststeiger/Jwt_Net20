    ...
    public sealed class SignAndVerify
    {
        ...
        */
			...
			bool	compress)
        {
            if (armor)
            {
                outputStream = new Org.BouncyCastle.Bcpg.ArmoredOutputStream(outputStream);
            }

            PgpSecretKey pgpSec = PgpExampleUtilities.ReadSecretKey(keyIn);
            PgpPrivateKey pgpPrivKey = pgpSec.ExtractPrivateKey(pass);
            PgpSignatureGenerator sGen = new PgpSignatureGenerator(pgpSec.PublicKey.Algorithm, Org.BouncyCastle.Bcpg.HashAlgorithmTag.Sha1);
			...
			if (compress)
			{
                cGen = new PgpCompressedDataGenerator(Org.BouncyCastle.Bcpg.CompressionAlgorithmTag.ZLib);

				cOut = cGen.Open(cOut);
			}

            Org.BouncyCastle.Bcpg.BcpgOutputStream bOut = new Org.BouncyCastle.Bcpg.BcpgOutputStream(cOut);
